...
y1_model=multinom_reg(penalty=0) %>% set_engine("glmnet") %>% set_mode("classification")
wf=workflow() %>% add_recipe(yr1_recipe) %>% add_model(y1_model)
tuning=wf %>% tune_grid(resamples=cv,metrics=metric_set(accuracy))
wf=wf %>% finalize_workflow(tuning %>% select_best(metric="accuracy"))
...
```{r knn_yrs_indep}
y1_model=nearest_neighbor(neighbors = tune()) %>% 
  set_engine("kknn") %>% set_mode("classification")
wf=workflow() %>% add_recipe(yr1_recipe) %>% add_model(y1_model)
tuning=wf %>% tune_grid(resamples=cv,
                        grid=expand.grid(neighbors=5:50),
                        metrics=metric_set(accuracy))
wf=wf %>% finalize_workflow(tuning %>% select_best(metric="accuracy"))
knn_fit=fit(wf,train_set)
tuning %>% select_best(metric="accuracy") %>% knitr::kable()
```

Looks like the best knn model is right in the middle of the tune grid with using the 26 closest comparable players.
...
y1_model=decision_tree(cost_complexity = tune(),min_n=tune()) %>% 
  set_engine("rpart") %>% set_mode("classification")
wf=workflow() %>% add_recipe(yr1_recipe) %>% add_model(y1_model)
tuning=wf %>% tune_grid(resamples=cv,
                        grid=expand.grid(cost_complexity=seq(0.01,0.05,by=0.01),
                                         min_n=seq(1:10)),
                        metrics=metric_set(accuracy))
wf=wf %>% finalize_workflow(tuning %>% select_best(metric="accuracy"))
...
set.seed(100,sample.kind = "Rounding")
y1_model=rand_forest(mtry = tune()) %>% 
...
```{r svm_yrs_indep}
set.seed(100,sample.kind = "Rounding")
y1_model=svm_rbf(rbf_sigma=tune(),cost=tune()) %>% 

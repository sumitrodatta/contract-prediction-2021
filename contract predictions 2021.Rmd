...
get_yrs_metrics<-function(model,model_name,tuning_var){
  f_meas=tuning_var %>% collect_metrics() %>% pull(mean) %>% max(.)
  contract_yr_predict_vec=predict(model,new_data=train_set) %>% pull()
  retrain_acc=accuracy_vec(truth=train_set$contract_yrs,
                           estimate=contract_yr_predict_vec)
  off_by_more_than_one=sum(abs(as.numeric(as.character(train_set$contract_yrs))-
...
multinom_tuning=wf %>% tune_grid(resamples=cv,metrics=metric_set(f_meas))
wf=wf %>% finalize_workflow(multinom_tuning %>% select_best(metric="f_meas"))
multinom_fit=fit(wf,train_set)
```

Let's get the most important variables of the multinomial model.

```{r multinom_top_vars,echo=FALSE}
vip(pull_workflow_fit(multinom_fit),scale=TRUE)
```
...
knn_tuning=wf %>% tune_grid(resamples=cv,
                        grid=expand.grid(neighbors=5:50),
                        metrics=metric_set(f_meas))
wf=wf %>% finalize_workflow(knn_tuning %>% select_best(metric="f_meas"))
knn_fit=fit(wf,train_set)
knn_tuning %>% select_best(metric="f_meas") %>% knitr::kable()
```
...
tree_tuning=wf %>% tune_grid(resamples=cv,
                        grid=expand.grid(cost_complexity=0.1^seq(2,5),
                                         min_n=seq(1:10)),
                        metrics=metric_set(f_meas))
wf=wf %>% finalize_workflow(tree_tuning %>% select_best(metric="f_meas"))
decision_tree_fit=fit(wf,train_set)
tree_tuning %>% select_best(metric="f_meas") %>% knitr::kable()
```
Let's get the most important variables of the decision_tree model.

```{r decision_tree_top_vars,echo=FALSE}
vip(pull_workflow_fit(decision_tree_fit),scale=TRUE)
```
...
rpart.plot(pull_workflow_fit(decision_tree_fit)$fit)
```
...
forest_tuning=wf %>% tune_grid(resamples=cv,
                        grid=expand.grid(mtry=5:10),
                        metrics=metric_set(f_meas))
wf=wf %>% finalize_workflow(forest_tuning %>% select_best(metric="f_meas"))
rand_forest_fit=fit(wf,train_set)
forest_tuning %>% select_best(metric="f_meas") %>% knitr::kable()
```

```{r rand_forest_top_vars,echo=FALSE}
vip(pull_workflow_fit(rand_forest_fit),scale=TRUE)
```
...
svm_tuning=wf %>% tune_grid(resamples=cv,
                        grid=expand.grid(rbf_sigma=0.1^seq(2:5),
                                         cost=c(0.25,0.5,1)),
                        metrics=metric_set(f_meas))
wf=wf %>% finalize_workflow(svm_tuning %>% select_best(metric="f_meas"))
svm_fit=fit(wf,train_set)
svm_tuning %>% select_best(metric="f_meas") %>% knitr::kable()
```
...
models<-get_yrs_metrics(multinom_fit,"Multinomial",multinom_tuning)
models<-bind_rows(models,
                  get_yrs_metrics(knn_fit,"KNN",knn_tuning),
                  get_yrs_metrics(decision_tree_fit,"Decision Tree",tree_tuning),
                  get_yrs_metrics(rand_forest_fit,"Random Forest",forest_tuning),
                  get_yrs_metrics(svm_fit,"SVM",svm_tuning))
models %>% knitr::kable()
```

The SVM suffers from the same problem that I talked about when fitting the decision tree. It has the highest F1 score, but the lowest accuracy. In fact, more of its predictions are more than 1 year off than being correct. The random forest stands far above the rest with 97% accuracy! It also has the highest F1-score barring the SVM. In last year's project, the random forests had the best performance as well, but had difficulty distinguishing 5-year contracts. Shifting to a classification problem seems to have solved that. The KNN model pipped the multinomial model in F1-score and accuracy, but had significantly less max-year predictions. While anticlimactic, I think the best course of action is to simply use the random forest model to make contract-year predictions. We will re-do the cross validation, but this time stratify on the years, which are now a dependent variable.

```{r y1_predict_for_s2}
train_set_after_y1=train_set %>% select(-contract_yrs) %>% 
  bind_cols(contract_yrs=predict(rand_forest_fit,newdata=train_set))
cv=vfold_cv(train_set_after_y1,v=10,strata=contract_yrs)
